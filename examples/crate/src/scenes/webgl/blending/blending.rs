use awsm::webgl::{Id, GlToggle, BlendFactor, ClearBufferMask, UniformLocation, AttributeLocation, SimpleTextureOptions, WebGlTextureSource, PixelFormat, WebGlRenderer, Uniform, BeginMode};
...
fn render(state:&State, webgl_renderer:&mut WebGlRenderer, is_top: bool) -> Result<(), JsValue> {
    ...
    webgl_renderer.set_depth_mask(false); // could also use depth_mask?
    webgl_renderer.toggle(GlToggle::Blend, true);
    webgl_renderer.set_blend_func(BlendFactor::SrcAlpha, BlendFactor::OneMinusSrcAlpha);

    //enable texture
    webgl_renderer.activate_texture_for_sampler(target_texture_id, 0)?;
